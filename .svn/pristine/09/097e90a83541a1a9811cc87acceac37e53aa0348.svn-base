...
public class ReservController {
	...
	public ModelAndView payResult(HttpSession session) {
	...
	public ModelAndView reservPayCart(HttpSession session, RedirectAttributes redirectAttributes) {
		LoginUser loginUser = (LoginUser)session.getAttribute("loginUser");
		// 로그인 여부 확인
		if (loginUser == null) {
			ModelAndView mav = new ModelAndView("main");
		...
		for(CartItem cartItem : cartItems ) {
			//장바구니 결제 시 중복된 예약이 있는지 확인
			Integer checkReserv = this.cartDao.checkSameReserv(cartItem);
			if(checkReserv >= 1	) { //장바구니의 객실의 예약이 존재할 때
				redirectAttributes.addFlashAttribute("message", "장바구니 객실의 해당 날짜 예약이 이미 존재합니다.");
				return new ModelAndView("redirect:/cart/cartList.html");
			}
			else { //장바구니의 객실의 예약이 존재하지 않을 때
